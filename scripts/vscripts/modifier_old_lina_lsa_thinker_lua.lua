modifier_old_lina_lsa_thinker_lua = class({})

--------------------------------------------------------------------------------

function modifier_old_lina_lsa_thinker_lua:IsHidden()
	return true
end

--------------------------------------------------------------------------------

function modifier_old_lina_lsa_thinker_lua:OnCreated( kv )
...
function modifier_old_lina_lsa_thinker_lua:OnIntervalThink()
	if IsServer() then
		...
		if #enemies > 0 then
			for _,enemy in pairs(enemies) do
				if enemy ~= nil and ( not enemy:IsMagicImmune() ) and ( not enemy:IsInvulnerable() ) then
					...
					ApplyDamage( damage )
					enemy:AddNewModifier( self:GetCaster(), self:GetAbility(), "modifier_old_lina_lsa_lua", { duration = self.light_strike_array_stun_duration } )
				end
			end
		end
