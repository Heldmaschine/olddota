modifier_razor_chain_lightning_thinker_lua = class({})

--------------------------------------------------------------------------------

function modifier_razor_chain_lightning_thinker_lua:IsHidden()
	return true
end

--------------------------------------------------------------------------------

function modifier_razor_chain_lightning_thinker_lua:OnCreated( kv )
	...
	self.jump_radius = self:GetAbility():GetSpecialValueFor( "radius" )
...
function modifier_razor_chain_lightning_thinker_lua:OnDestroy()
	if IsServer() then
...
function modifier_razor_chain_lightning_thinker_lua:OnIntervalThink()
	if IsServer() then
...
function modifier_razor_chain_lightning_thinker_lua:GetNextTarget()
	local enemies = FindUnitsInRadius( self:GetCaster():GetTeamNumber(), self:GetParent():GetOrigin(), nil, jump_radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_CREEP, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, 0, false )
...
function modifier_razor_chain_lightning_thinker_lua:HitTarget( hTarget )
	if hTarget == nil then
		return
	end
...
function modifier_razor_chain_lightning_thinker_lua:CreateBounceFX( hTarget )
	--FX
