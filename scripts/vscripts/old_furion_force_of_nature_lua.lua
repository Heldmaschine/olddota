old_furion_force_of_nature_lua = class({})
LinkLuaModifier( "modifier_old_furion_force_of_nature_lua", LUA_MODIFIER_MOTION_NONE )

--------------------------------------------------------------------------------

function old_furion_force_of_nature_lua:GetAOERadius()
	return self:GetSpecialValueFor( "area_of_effect" )
end

--------------------------------------------------------------------------------

function old_furion_force_of_nature_lua:IsHiddenWhenStolen()
	return true
end

--------------------------------------------------------------------------------

function old_furion_force_of_nature_lua:OnSpellStart()
	...
	while nTreantsToSpawn > 0 do
		local hTreant = CreateUnitByName( "npc_old_furion_treant", vTargetPosition, true, self:GetCaster(), self:GetCaster():GetOwner(), self:GetCaster():GetTeamNumber() )
		if hTreant ~= nil then
			...
			hTreant:AddNewModifier( self:GetCaster(), self, "modifier_old_furion_force_of_nature_lua", kv )
		end
