modifier_old_razor_chain_lightning_thinker_lua = class({})

--------------------------------------------------------------------------------

function modifier_old_razor_chain_lightning_thinker_lua:IsHidden()
	return true
end

--------------------------------------------------------------------------------

function modifier_old_razor_chain_lightning_thinker_lua:OnCreated( kv )
	...
	if IsServer() then
		...
		if self.hTarget == nil then
			...
			local nFXIndexStart = ParticleManager:CreateParticle( "particles/units/heroes/hero_zuus/zuus_arc_lightning_head.vpcf", PATTACH_CUSTOMORIGIN, nil )
...
function modifier_old_razor_chain_lightning_thinker_lua:OnDestroy()
	if IsServer() then
...
function modifier_old_razor_chain_lightning_thinker_lua:OnIntervalThink()
	if IsServer() then
...
function modifier_old_razor_chain_lightning_thinker_lua:GetNextTarget()
	local enemies = FindUnitsInRadius( self:GetCaster():GetTeamNumber(), self:GetParent():GetOrigin(), nil, self.radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_CREEP, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, 0, false )
...
function modifier_old_razor_chain_lightning_thinker_lua:HitTarget( hTarget )
	if hTarget == nil then
		return
	end
...
function modifier_old_razor_chain_lightning_thinker_lua:CreateBounceFX( hTarget )
	--FX
	...
	local nFXIndexHit = ParticleManager:CreateParticle( "particles/units/heroes/hero_zuus/zuus_arc_lightning_head.vpcf", PATTACH_CUSTOMORIGIN, nil );
